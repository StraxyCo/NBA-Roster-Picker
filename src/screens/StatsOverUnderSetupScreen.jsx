...
export const ALL_STATS = [
  ...
  { key: 'fg3m', label: '3PM',         unit: '3PM' },
  { key: 'gp',   label: 'Games Played',unit: 'GP'  },
]
      ...
      <div className={styles.playerPickList}>
        ...
        {available.map(p => (
          <button key={p.id} className={styles.playerPickRow} onClick={() => { onSelect(p); onClose() }}>
            ...
            <span className={styles.playerPickStat}>{p.stats?.statsOverUnder?.played || 0} games</span>
          </button>
        ))}
      </div>
...
function StatsView({ players, onClose }) {
  const sorted = [...players].filter(p => (p.stats?.statsOverUnder?.played || 0) > 0)
    .sort((a, b) => (b.stats?.statsOverUnder?.wins || 0) - (a.stats?.statsOverUnder?.wins || 0))
  return (
    <Modal onClose={onClose}>
      <h3 className={styles.modalTitle}>Stats Over/Under Stats</h3>
      <div className={styles.statsTable}>
        <div className={styles.statsHeader}>
          <span className={styles.statsColPlayer}>Player</span>
          <span className={styles.statsCol}>GP</span><span className={styles.statsCol}>W</span><span className={styles.statsCol}>Win%</span>
        </div>
        {sorted.length === 0 && <p className={styles.emptyNote}>No games recorded yet.</p>}
        {sorted.map(p => {
          const gp = p.stats?.statsOverUnder?.played || 0
          const w  = p.stats?.statsOverUnder?.wins   || 0
          return (
  ...
  return (
    <div className={styles.container}>
      ...
      <header className={styles.header}>
        <h1 className={styles.title}>Stats Over/Under</h1>
        <p className={styles.subtitle}>Bet over or under on player stats</p>
      </header>
      ...
      <div className={styles.mainContainer}>
        ...
        <div className={styles.section}>
          ...
          <div className={styles.optionRow}>
            <div className={styles.optionLabel}>
              ...
              <span className={styles.optionDesc}>Players drawn per user</span>
            </div>
        ...
        {/* Stats selector */}
        <div className={styles.section}>
          <h2 className={styles.sectionLabel}>Stats ({selectedStats.length}/{ALL_STATS.length} selected)</h2>
