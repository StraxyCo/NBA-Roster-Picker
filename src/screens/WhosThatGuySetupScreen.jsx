      ...
      <div className={styles.playerPickList}>
        ...
        {available.map(p => (
          <button key={p.id} className={styles.playerPickRow} onClick={() => { onSelect(p); onClose() }}>
            ...
            <span className={styles.playerPickStat}>{p.stats?.whosThatGuy?.played || 0} games</span>
          </button>
        ))}
      </div>
...
function StatsView({ players, onClose }) {
  const sorted = [...players].filter(p => (p.stats?.whosThatGuy?.played || 0) > 0)
    .sort((a, b) => (b.stats?.whosThatGuy?.wins || 0) - (a.stats?.whosThatGuy?.wins || 0))
  return (
    <Modal onClose={onClose}>
      <h3 className={styles.modalTitle}>Who's That Guy? Stats</h3>
      <div className={styles.statsTable}>
        ...
        {sorted.map(p => {
          const gp = p.stats?.whosThatGuy?.played || 0
          const w  = p.stats?.whosThatGuy?.wins   || 0
          return (
  ...
  return (
    <div className={styles.container}>
      ...
      <header className={styles.header}>
        <h1 className={styles.title}>Who's That Guy?</h1>
        <p className={styles.subtitle}>Identify a player from their career stat line</p>
      </header>
        ...
        <div className={styles.section}>
          ...
          <div className={styles.optionRow}>
            <div className={styles.optionLabel}>
              ...
              <span className={styles.optionDesc}>Players drawn per person</span>
            </div>
      ...
      <button
        ...
        disabled={!canStart}
        onClick={() => onStart({ players: activePlayers, rounds, minSeasons, seasons, showTeams })}
      >
        Start Game
      </button>
