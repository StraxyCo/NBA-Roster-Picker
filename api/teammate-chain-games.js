...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      const raw = await redisGet('teammate-chain-games')
    ...
    if (req.method === 'POST') {
      ...
      games.push(newGame)
      await redisSet('teammate-chain-games', JSON.stringify(games))
      for (const pid of playerIds) {
        const pRaw = await hget('players', pid)
        if (!pRaw) continue
        const player = normalizePlayer(pRaw)
        if (!player.stats.teammateChain) player.stats.teammateChain = { played: 0, wins: 0 }
        player.stats.teammateChain.played++
        if (pid === winnerId) player.stats.teammateChain.wins++
    ...
    if (req.method === 'DELETE') {
      ...
      for (const pid of (game.playerIds || [])) {
        ...
        const player = normalizePlayer(pRaw)
        if (!player.stats.teammateChain) player.stats.teammateChain = { played: 0, wins: 0 }
        player.stats.teammateChain.played = Math.max(0, player.stats.teammateChain.played - 1)
        if (pid === game.winnerId) player.stats.teammateChain.wins = Math.max(0, player.stats.teammateChain.wins - 1)
        await hset('players', pid, JSON.stringify(player))
      }
    ...
    console.error('[teammate-chain-games] error:', err.message)
