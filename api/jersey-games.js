  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      for (const pid of playerIds) {
        ...
        player.jerseyGamesPlayed = (player.jerseyGamesPlayed || 0) + 1
        if (pid === winnerId) player.jerseyWins = (player.jerseyWins || 0) + 1
    ...
    if (req.method === 'DELETE') {
      ...
      for (const pid of (game.playerIds || [])) {
        ...
        if (!pRaw) continue
        const player = typeof pRaw === 'string' ? JSON.parse(pRaw) : pRaw
        player.jerseyGamesPlayed = Math.max(0, (player.jerseyGamesPlayed || 0) - 1)
        if (pid === game.winnerId) player.jerseyWins = Math.max(0, (player.jerseyWins || 0) - 1)
        await hset('players', pid, JSON.stringify(player))
      }
