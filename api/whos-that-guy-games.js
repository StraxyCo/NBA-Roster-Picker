...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      const raw = await redisGet('whos-that-guy-games')
    ...
    if (req.method === 'POST') {
      ...
      games.push(newGame)
      await redisSet('whos-that-guy-games', JSON.stringify(games))
      for (const pid of playerIds) {
        const pRaw = await hget('players', pid)
        if (!pRaw) continue
        const player = normalizePlayer(pRaw)
        if (!player.stats.whosThatGuy) player.stats.whosThatGuy = { played: 0, wins: 0 }
        player.stats.whosThatGuy.played++
        if (pid === winnerId) player.stats.whosThatGuy.wins++
    ...
    if (req.method === 'DELETE') {
      ...
      for (const pid of (game.playerIds || [])) {
        ...
        const player = normalizePlayer(pRaw)
        if (!player.stats.whosThatGuy) player.stats.whosThatGuy = { played: 0, wins: 0 }
        player.stats.whosThatGuy.played = Math.max(0, player.stats.whosThatGuy.played - 1)
        if (pid === game.winnerId) player.stats.whosThatGuy.wins = Math.max(0, player.stats.whosThatGuy.wins - 1)
        await hset('players', pid, JSON.stringify(player))
      }
    ...
    console.error('[whos-that-guy-games] error:', err.message)
