...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      const raw = await redisGet('all-stars-games')
    ...
    if (req.method === 'POST') {
      ...
      games.push(newGame)
      await redisSet('all-stars-games', JSON.stringify(games))
      for (const pid of playerIds) {
        const pRaw = await hget('players', pid)
        if (!pRaw) continue
        const player = normalizePlayer(pRaw)
        if (!player.stats.allStars) player.stats.allStars = { played: 0, wins: 0 }
        player.stats.allStars.played++
        if (pid === winnerId) player.stats.allStars.wins++
    ...
    if (req.method === 'DELETE') {
      ...
      for (const pid of (game.playerIds || [])) {
        ...
        const player = normalizePlayer(pRaw)
        if (!player.stats.allStars) player.stats.allStars = { played: 0, wins: 0 }
        player.stats.allStars.played = Math.max(0, player.stats.allStars.played - 1)
        if (pid === game.winnerId) player.stats.allStars.wins = Math.max(0, player.stats.allStars.wins - 1)
        await hset('players', pid, JSON.stringify(player))
      }
    ...
    console.error('[all-stars-games] error:', err.message)
