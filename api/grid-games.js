...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      const raw = await redisGet('grid-games')
    ...
    if (req.method === 'POST') {
      ...
      games.push(newGame)
      await redisSet('grid-games', JSON.stringify(games))
      for (const pid of playerIds) {
        const pRaw = await hget('players', pid)
        if (!pRaw) continue
        const player = normalizePlayer(pRaw)
        if (!player.stats.grid) player.stats.grid = { played: 0, wins: 0 }
        player.stats.grid.played++
        if (pid === winnerId) player.stats.grid.wins++
    ...
    if (req.method === 'DELETE') {
      ...
      for (const pid of (game.playerIds || [])) {
        ...
        const player = normalizePlayer(pRaw)
        if (!player.stats.grid) player.stats.grid = { played: 0, wins: 0 }
        player.stats.grid.played = Math.max(0, player.stats.grid.played - 1)
        if (pid === game.winnerId) player.stats.grid.wins = Math.max(0, player.stats.grid.wins - 1)
        await hset('players', pid, JSON.stringify(player))
      }
    ...
    console.error('[grid-games] error:', err.message)
